# subcategory-aware-rag-retrieval

## Purpose

Makes RAG-based question generation sub-category aware: retrieval is scoped per sub-category when sub-categories are defined for the requested category+difficulty, generated question counts are distributed across the available sub-categories, and sub-category descriptions are injected into the LLM prompt as generation guidance. Falls back to existing single-pool behavior when no sub-categories are defined.
